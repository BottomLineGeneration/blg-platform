...
function emptyClient(id) {
  return { id, name:id, fullName:"", contact:"", locations:0, capRate:0.065, services:[], files:[], notes:"" };
}
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  body{font-family:'Inter',sans-serif;color:#1A2B4A;background:#fff}
  ...
  .kpi.green .val{color:#00875A} .kpi.amber .val{color:#B87A00} .kpi.blue .val{color:#0050B3}
...
<div class="section">
  ...
  <table>
    <thead><tr><th>Location</th><th>Type</th><th>Vendor</th><th>Monthly Cost</th><th>Proposed</th><th>Annual Savings</th><th>% Savings</th></tr></thead>
    <tbody>
...
<div class="footer">Bottom Line Generation &nbsp;·&nbsp; Byron Braun &nbsp;·&nbsp; 678.852.3928 &nbsp;·&nbsp; byronbraun@bottomlinegeneration.com &nbsp;·&nbsp; www.bottomlinegeneration.com</div>
...
export default function App() {
  ...
  const [chat, setChat]             = useState([{role:"assistant", content:`Hello! I'm your BLG Intelligence Analyst. Upload invoices and documents on the Upload tab to start building the inventory for **${clientId}**. I'll extract all the data automatically.`}]);
  ...
  const totalSavings = services.reduce((s,r)=>s+(r.savings||0),0);
    ...
    for (const fo of objs) {
      ...
      try {
        ...
        if (fo.file.type==="application/pdf" || fo.name.endsWith(".pdf")) {
          ...
          msgContent = [{type:"document",source:{type:"base64",media_type:"application/pdf",data:b64}},{type:"text",text:EXTRACT_PROMPT}];
        } else if (fo.file.type.startsWith("image/")) {
          const b64 = await new Promise((res,rej)=>{const r=new FileReader();r.onload=()=>res(r.result.split(",")[1]);r.onerror=rej;r.readAsDataURL(fo.file)});
          msgContent = [{type:"image",source:{type:"base64",media_type:fo.file.type,data:b64}},{type:"text",text:EXTRACT_PROMPT}];
        } else {
        ...
        if (extracted && Array.isArray(extracted) && extracted.length>0) {
          setClient(prev=>({...prev, services:[...(prev.services||[]),...extracted.map(svc=>({id:Date.now()+Math.random(),...svc,source:fo.name,addedAt:new Date().toISOString()}))]}));
        }
      } catch(e) {
  ...
  const sendChat = async () => {
    ...
    const systemPrompt = `You are the BLG Intelligence Analyst — expert in Telecom Expense Management AND Utility optimization for REIT and commercial real estate portfolios.

Client: ${client.fullName||client.name||clientId}
Locations: ${locations} | Services: ${services.length}
Monthly spend: ${fmt$(totalMonthly)} | Monthly savings: ${fmt$(totalSavings)}
Annual savings: ${fmtK(totalAnnual)} | NOI lift: ${fmtK(noi)} | Valuation impact: ${fmtK(valuation)}
...
Be specific, data-driven, professional. Tie everything back to NOI and valuation impact.`;
    ...
    <div style={s.app}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        input:focus,select:focus,textarea:focus{outline:none;border-color:${C.amber}!important}
          ...
          <div>
            ...
            {isEmpty ? (
              <div style={{textAlign:"center",padding:"80px 40px"}}>
                ...
                <div style={{fontSize:14,color:C.muted,marginBottom:32,maxWidth:480,margin:"0 auto 32px"}}>Upload telecom invoices, utility bills, or contracts. The AI will extract everything automatically.</div>
              ...
              <>
                ...
                <div style={s.card()}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                    ...
                    <button style={s.btn("amber")} onClick={()=>exportReport(client,summary)}>⬇ Download Report</button>
                  </div>
                      ...
                      {[...new Set(services.map(r=>r.location).filter(Boolean))].map(loc=>{
                        ...
                        return (
                          <tr key={loc}>
                            ...
                            <td style={s.td()}>{pct>0&&<><span style={{color:pct>.3?C.green:C.amber,fontWeight:700}}>{fmtPct(pct)}</span><Bar pct={pct*100} color={pct>.3?C.green:C.amber}/></>}</td>
                          </tr>
                        );
                      })}
                    </tbody>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
              <div>
                <div style={{border:`2px dashed ${dragOver?C.amber:C.border}`,borderRadius:12,padding:"40px 24px",textAlign:"center",cursor:"pointer",background:dragOver?`${C.amber}08`:"transparent",transition:"all .2s",marginBottom:14}}
                  ...
                  <div style={{fontSize:12,color:C.muted,marginBottom:16}}>Electric · Gas · Water · Waste · POTS · Internet · UCaaS · Contracts</div>
                ...
                <div style={s.card()}>
                  ...
                  {[
                    ["⚡","Electric Bills","Utility, rate, usage, contract dates"],
                    ["🔥","Gas Bills","Provider, usage, rates, contract terms"],
                    ["💧","Water & Sewer","Meter info, tier rates, usage history"],
                    ["♻️","Waste / Recycling","Hauler, frequency, monthly cost"],
                    ["📞","POTS / Analog Lines","Carrier, phone numbers, per-line cost"],
                    ["🌐","Internet / Circuits","Provider, speed, monthly rate, contract end"],
                    ...
                    {f.extracted&&Array.isArray(f.extracted)&&(
                      <div style={{marginTop:8,padding:"8px 10px",background:`${C.green}08`,borderRadius:6,border:`1px solid ${C.green}20`}}>
                        ...
                        {f.extracted.slice(0,3).map((e,i)=>(
                          <div key={i} style={{fontSize:11,color:C.muted}}>{e.type||"Service"} · {e.location||"?"} · {e.vendor||"?"} · {fmt$(e.monthly)}/mo</div>
                        ))}
        ...
        {tab==="analysis" && (
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
              ...
              {!isEmpty&&<button style={s.btn("amber")} onClick={()=>exportReport(client,summary)}>⬇ Download Report</button>}
            </div>
            {isEmpty ? (
              <div style={{...s.card({textAlign:"center",padding:"60px 40px"})}}>
                <div style={{fontSize:36,marginBottom:12}}>📐</div>
              ...
              <>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:16}}>
                  ...
                  <div style={s.card()}>
                    ...
                    {[
                      {icon:"💰",title:"OPEX Reduction",    desc:`${fmtK(totalAnnual)}/year identified across ${services.length} services — zero capex required.`},
                      {icon:"📈",title:"NOI Enhancement",   desc:`${fmtK(noi)}/year NOI improvement. Direct contribution to portfolio operating performance.`},
                      {icon:"🏗️",title:"Valuation Impact",  desc:`At ${(capRate*100).toFixed(1)}% cap rate: ${fmtK(valuation)} in incremental asset value.`},
                      {icon:"📊",title:"No Upfront Cost",   desc:`100% performance-based model. BLG fees tied to verified savings only.`},
                    ].map(pt=>(
